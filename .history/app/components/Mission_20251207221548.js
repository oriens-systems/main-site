...
export default function Mission() {
  ...
  const shapeRotate = useTransform(scrollYProgress, [0, 1], [0, 15]);
  ...
  return (
    ...
    >
      {/* Grid Background - Rotated, Animated & Parallax */}
      <motion.div
      ...
      <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 items-center min-h-screen max-w-[1800px] mx-auto px-8 md:px-12 lg:px-16 py-20 gap-8">
        {/* Left Side - 3D Shape with Parallax */}
        <motion.div
        ...
        >
          ...
          >
            {features.map((feature, index) => (
              <motion.div
                key={feature.label}
                className="flex items-center gap-3 px-6 py-4 bg-black rounded-lg border border-white/10 hover:border-[#5900A7]/50 transition-all duration-300"
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
